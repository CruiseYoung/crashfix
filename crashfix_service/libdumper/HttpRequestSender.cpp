//! \file HttpRequestSender.cpp
//! \brief Sends error report over HTTP connection.
...
{ 
	...
	{
		m_Assync->SetProgress(_T("Determining delivery status from HTTP response body."), 0);

		// Get status code from HTTP response
		if(atoi((LPCSTR)pBuffer)!=200)
		{
			m_Assync->SetProgress(_T("Return code indicates an error (expected code 200)."), 100, false);
			goto cleanup;
		}
	}
	else
	...
	// Add a message to log
    m_Assync->SetProgress(_T("File has been uploaded OK!"), 100, false);
    bStatus = TRUE;
